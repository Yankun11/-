<template>
    <div class="chart">
        ...
        <ul class="navigator">
            <li @click="changePage(inedx)" :class="{ active: index === currentIndex }" class="navigatorItem"
                v-for="(item, index) in quanguoTrendChart" :key="index">
                {{ item.title }}
            </li>
        </ul>
    </div>
...
export default {
    ...
    methods: {
        changePage(index) {
            this.xurrentIndex = index  //高亮
            this.$refs.mySwiper.swiper.slideTo(index, 1000, false)  //swiper的切换
        }
    }
}
</script>
